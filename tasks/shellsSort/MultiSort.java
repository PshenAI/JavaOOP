...
public class MultiSort {
	...
	static void sort(int[] array, int threadNumber) {
		ShellsSort[] threadarray = new ShellsSort[threadNumber];
		for (int i = 0; i < threadarray.length; i++) {
			...
			threadarray[i] = new ShellsSort(array, begin, end);
		}
		for (int i = 0; i < threadarray.length; i++) {
			try {
	...
	private static int[] mergeArrays(int[] array, ShellsSort[] threadarray) {
